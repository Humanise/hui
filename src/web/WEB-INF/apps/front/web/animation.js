hui.onReady(function() {
	...
	for (var i=0; i < text.length; i++) {
		chars.push(hui.build('span',{text:text[i],parent:title,style:{opacity:0,position:'relative',top:Math.round((Math.random()*100-50))+'px'}}));
	};
	...
	window.setTimeout(function() {
		...
		hui.animate({node:slogan,css:{opacity:1,paddingTop:'0px',letterSpacing:'1px'},delay:6000,ease:hui.ease.fastSlow,duration:2000})
		...
		function animateTitle() {
			for (var i=0; i < chars.length; i++) {
				...
				var char = chars[i];
				hui.animate({node:char,css:{opacity:1,top:'0px'},delay:1500+delay,ease:hui.ease.elastic,duration:2000-delay})
