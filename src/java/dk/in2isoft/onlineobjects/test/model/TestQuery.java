...
public class TestQuery extends AbstractTestCase {
	...
	public void testBasic() throws EndUserException {
		...
		assertEquals("select obj from dk.in2isoft.onlineobjects.model.Person as obj left join fetch obj.properties where obj.id>0 order by obj.name asc", hql);
	}
	...
	public void testFieldIn() throws EndUserException {
		...
		String hql = queryToHql(query);
		assertEquals("select obj from dk.in2isoft.onlineobjects.model.Word as obj left join fetch obj.properties where obj.id>0 and text in (:text) order by obj.name asc", hql);
	}
